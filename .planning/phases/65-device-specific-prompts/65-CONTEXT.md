...
- Dual-DSP routing is the #1 priority for the Helix family planner prompt — every Helix/LT/Floor/Rack preset uses both DSPs, and the old model routinely failed to connect them. The prompt must spell out exactly how blocks flow across DSP0 → DSP1
